...
export async function POST(req: Request) {
  ...
  if (!postId || !authorName || content.length < 2) return NextResponse.json({ error: 'Missing fields' }, { status: 400 });
  const db = getDB();
  if (!db) return NextResponse.json({ error: 'Database unavailable' }, { status: 503 });
  const since = new Date(Date.now() - 3_600_000).toISOString();
  const { results: recent } = await db.prepare(
    'SELECT COUNT(*) as cnt FROM comments WHERE author_name = ? AND created_at > ?'
  ).bind(authorName, since).all();
  if ((recent[0]?.cnt as number) >= 5) return NextResponse.json({ error: 'Rate limited' }, { status: 429 });
  const id = nanoid();
  await db.prepare('INSERT INTO comments (id, post_id, author_name, content) VALUES (?, ?, ?, ?)').bind(id, postId, authorName, content).run();
